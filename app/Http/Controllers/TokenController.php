...
class TokenController extends Controller
{
    ...
    function Create(Request $request): JsonResponse
    {
        ...
        $TransmittedToken = UserToken::UserWhere(Auth::id())->Token($request->header('X-token'))->firstOrFail();
        ...
        $token = UserToken::UserWhere(Auth::id())->Token($token)->first();
    ...
    function VerifiAllowTeamspeakVirtualServers(Collection $AllowTeamspeakVirtualServers, array $RequestedTeamSpeakVirtualServer): void
    {
        foreach ($RequestedTeamSpeakVirtualServer as $key) {
            ...
            if ($Found != true) {
                echo 'error no allow teamspeak virtual server' . '<br/>';
                //TODO реализовать исключение (Запрашиваемая привелегия не разрешена)
            }
        }
