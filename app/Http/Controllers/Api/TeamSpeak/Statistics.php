...
class Statistics extends Controller
{
     ...
     */
    function GetInstanseStatisticsYear($server_id)
    {
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    function GetInstanseStatisticsMonth($server_id)
    {
        $this->server_id = $server_id;
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    function GetInstanseStatisticsWeek($server_id)
    {
        $this->server_id = $server_id;
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    function GetInstanseStatisticsDay($server_id)
    {
        $this->server_id = $server_id;
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    function GetInstanseStatisticsRealtime($server_id)
    {
        $this->server_id = $server_id;
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    function GetVirtualServerStatisticsYear($server_id, $uid)
    {
        $this->server_id = $server_id;
        ...
        $data = Cache::remember('VirtualServerStatisticsYearServerID-' . $server_id . '-VirtualServerUID-' . $this->uid, Carbon::now()->addMinutes(config('ApiCache.TeamSpeak.Statistics.VirtualServer.Year')), function () {
            return StatisticsVirtualServers::ServerID($this->server_id)->VirtualServerUID($this->uid)->StatYear()->DayAvage()->get();
        });
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    function GetVirtualServerStatisticsMonth($server_id, $uid)
    {
        $this->server_id = $server_id;
        $this->uid = base64_decode($uid);
        ...
        $data = Cache::remember('VirtualServerStatisticsMonthServerID-' . $server_id . '-VirtualServerUID-' . $this->uid, Carbon::now()->addMinutes(config('ApiCache.TeamSpeak.Statistics.VirtualServer.Month')), function () {
            return StatisticsVirtualServers::ServerID($this->server_id)->VirtualServerUID($this->uid)->StatMonth()->HourAvage()->get();
        });
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    function GetVirtualServerStatisticsWeek($server_id, $uid)
    {
        $this->server_id = $server_id;
        $this->uid = base64_decode($uid);
        ...
        $data = Cache::remember('VirtualServerStatisticsWeekServerID-' . $server_id . '-VirtualServerUID-' . $this->uid, Carbon::now()->addMinutes(config('ApiCache.TeamSpeak.Statistics.VirtualServer.Week')), function () {
            return StatisticsVirtualServers::ServerID($this->server_id)->VirtualServerUID($this->uid)->StatWeek()->HalfHourAvage()->get();
        });
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    function GetVirtualServerStatisticsDay($server_id, $uid)
    {
        $this->server_id = $server_id;
        $this->uid = base64_decode($uid);
        ...
        $data = Cache::remember('VirtualServerStatisticsDayServerID-' . $server_id . '-VirtualServerUID-' . $this->uid, Carbon::now()->addMinutes(config('ApiCache.TeamSpeak.Statistics.VirtualServer.Day')), function () {
            return StatisticsVirtualServers::ServerID($this->server_id)->VirtualServerUID($this->uid)->StatDay()->FiveMinutesAvage()->get();
        });
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    function GetVirtualServerStatisticsRealtime($server_id, $uid)
    {
        $this->server_id = $server_id;
        $this->uid = base64_decode($uid);
        ...
        $data = Cache::remember('VirtualServerStatisticsRealtimeServerID-' . $server_id . '-VirtualServerUID-' . $this->uid, Carbon::now()->addMinutes(config('ApiCache.TeamSpeak.Statistics.VirtualServer.Realtime')), function () {
            return StatisticsVirtualServers::ServerID($this->server_id)->VirtualServerUID($this->uid)->StatRealtime()->get();
        });
