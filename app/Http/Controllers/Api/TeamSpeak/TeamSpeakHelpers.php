...
namespace Api\Http\Controllers\TeamSpeak;
...
class TeamSpeakHelpers extends Controller
{
     ...
     */
    function InstanceIsBlacklisted($server_id)
    {
        $BlacklistCheck = new BlacklistCheck();
        $data = $BlacklistCheck->isBlacklisted(Servers::find($server_id)->ipaddress);
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    function UpdateServerMirrorList()
    {
        $Updater = new Update;
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    function InstanceOutdatedCheck($server_id)
    {
        $ts3conn = new ts3query($server_id);;
        $OutdatedChecker = new Update;
        $data = $OutdatedChecker->OutdatedCheck((string)$ts3conn->version()['version'], $ts3conn->version()['platform']);
