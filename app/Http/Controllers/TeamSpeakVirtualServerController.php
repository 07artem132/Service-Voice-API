...
class TeamSpeakVirtualServerController extends Controller
{
     ...
     * @return JsonResponse
     */
    function Create(Request $request, int $server_id): JsonResponse
    {
        if (!$request->isJson())
            throw new RequestIsNotJson();

        $rules = config('ApiValidation.TeamSpeak.VirtualServer.Create.rules');
        $messages = config('ApiValidation.TeamSpeak.VirtualServer.Create.messages');

        $this->validate($request, $rules, $messages);

        $ts3conn = new TeamSpeak($server_id);

        $data = $ts3conn->VirtualServerCreate($request->all());
    ...
    function Delete(int $server_id, string $bashe64uid): JsonResponse
    {
        ...
        $ts3conn->VirtualServerDelete($uid);
    ...
    function ServerInfo(int $server_id, string $bashe64uid): JsonResponse
    {
        ...
        $data = $ts3conn->VirtualServerInfo();
